const YAHOO_UA = 'Mozilla/5.0 fin-new-sweep options endpoint';
...
async function fetchYahooAuth() {
  ...
  if (!cookie) throw new Error('Yahoo options auth unavailable');

  const crumbRes = await fetch('https://query1.finance.yahoo.com/v1/test/getcrumb', {
    headers: { cookie, 'user-agent': YAHOO_UA },
  });
  if (!crumbRes.ok) throw new Error(`Yahoo options auth ${crumbRes.status}`);
  const crumb = (await crumbRes.text()).trim();
  if (!crumb || crumb.startsWith('{')) throw new Error('Yahoo options crumb unavailable');
  return { cookie, crumb };
}
...
async function fetchOptionChain(ticker, auth, expiration = null) {
  ...
  if (!res.ok) {
    const err = new Error(`Yahoo options ${res.status}`);
    err.status = res.status;
    throw err;
  }
  const body = await res.json();
  const error = body?.optionChain?.error || body?.finance?.error;
  if (error) throw new Error(error.description || error.code || 'Yahoo options unavailable');
...
async function buildOptionsActivity(ticker, env) {
  const block = await kvGet(env, 'yahoo:options:blocked_until');
  if (block && Number(block) > Date.now()) throw new Error('Yahoo options refresh deferred');
...
export async function onRequestGet(context) {
  ...
  try {
    ...
    if ([403, 429, 500, 502, 503, 504].includes(err.status)) {
      await kvPut(env, 'yahoo:options:blocked_until', String(Date.now() + TTL.blocked * 1000), TTL.blocked);
    }
    return json({
      success: false,
      ticker,
